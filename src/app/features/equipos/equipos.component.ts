...
export class EquiposComponent implements OnInit {
  ...
  constructor(
    ...
    console.log('Servicio inyectado:', dataService);
  }
  ...
  cargarSeccion(): void {
    if (this.sectionId !== null) {
      this.http
        .get<any>(`http://localhost:3000/api/secciones/${this.sectionId}`)
        .subscribe(
          (data) => {
  ...
  cargarEquipos(): void {
    if (this.sectionId !== null) {
      this.http
        .get<any[]>(`http://localhost:3000/api/equipos/${this.sectionId}`)
        .subscribe(
          (data) => {
  ...
  editarEquipo(): void {
    ...
    this.http
      .put(
        `http://localhost:3000/api/equipos/${this.equipoEditar.id}`,
        this.nuevoEquipo
      )
      .subscribe(
        (response: any) => {
  ...
  onDelete(row: any): void {
    ...
    this.http.delete(`http://localhost:3000/api/equipos/${row.id}`).subscribe(
      (response) => {
        ...
        this.equiposData = this.equiposData.filter(
          (item) => item.id !== row.id
        );
      },
      (error) => {
